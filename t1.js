...
const handleSearch = () => {
    ...
    // Update the table with the searched items
...
const sortDate = () => {
    ...
    let newData;
    if (date_cat === "asc") {
        newData = data.sort((a, b) => new Date(a.date) - new Date(b.date));
    } else if (date_cat === "desc") {
        newData = data.sort((a, b) => new Date(b.date) - new Date(a.date));
    }
...
window.addEventListener("load", () => {
    ...
    if (data.length === 0) {
        ...
        todos.style.display = "block";
        sortDate(); // Call the sorting function on load
    }
});
